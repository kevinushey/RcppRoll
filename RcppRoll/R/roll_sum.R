...
roll_sum <- function( x, n, by.column=TRUE, weights=rep(1,n), normalize=FALSE ) {
	...
	if( is.matrix(x) ) {
		if( by.column ) {
			stopifnot( n <= nrow(x) )
			return( .Call( "RcppRoll_roll_sum_numeric_matrix", x, as.integer(n), as.numeric(weights), PACKAGE="RcppRoll" ) )
		} else {
			stopifnot( n <= ncol(x) )
			return( t( .Call( "RcppRoll_roll_sum_numeric_matrix", t(x), as.integer(n), as.numeric(weights), PACKAGE="RcppRoll" ) ) )
		}
	}
