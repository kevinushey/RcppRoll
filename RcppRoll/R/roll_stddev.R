#' Rolling Stddev
#'
#' This function implements a rolling stddev with C++/RcppArmadillo.
...
#' @export
roll_stddev <- function( x, n, by.column=TRUE ) {
	
	if( is.matrix(x) ) {
		if( n > nrow(x) ) {
			stop("n cannot be greater than nrow(x).")
		}
		return( .Call( "RcppRoll_roll_stddev_numeric_matrix", x, as.integer(n), as.logical(by.column), PACKAGE="RcppRoll" ) )
	}
	...
	if( is.vector(x) ) {
		...
		return( as.numeric( .Call( "RcppRoll_roll_stddev_numeric_vector", x, as.integer(n), PACKAGE="RcppRoll" ) ) )
	}
