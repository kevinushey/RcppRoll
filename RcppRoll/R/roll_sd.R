...
roll_sd <- function( x, n, by.column=TRUE ) {
	...
	if( is.matrix(x) ) {
		...
		return( .Call( "RcppRoll_roll_sd_numeric_matrix", x, as.integer(n), as.logical(by.column), PACKAGE="RcppRoll" ) )
	}
	...
	if( is.vector(x) ) {
		...
		return( as.numeric( .Call( "RcppRoll_roll_sd_numeric_vector", x, as.integer(n), PACKAGE="RcppRoll" ) ) )
	}
