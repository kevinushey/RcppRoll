...
roll_var <- function( x, n, by.column=TRUE, weights=rep(1,n), normalize=FALSE ) {
	...
	if( is.matrix(x) ) {
		if( by.column ) {
			stopifnot( n <= nrow(x) )
			return( .Call( "RcppRoll_roll_var_numeric_matrix", x, as.integer(n), as.numeric(weights), PACKAGE="RcppRoll" ) )
		} else {
			stopifnot( n <= ncol(x) )
			return( t( .Call( "RcppRoll_roll_var_numeric_matrix", t(x), as.integer(n), as.numeric(weights), PACKAGE="RcppRoll" ) ) )
		}
	}
	...
	if( is.vector(x) ) {
		...
		return( as.numeric( .Call( "RcppRoll_roll_var_numeric_vector", x, as.integer(n), as.numeric(weights), PACKAGE="RcppRoll" ) ) )
	}
