...
inline double var(std::vector<double> const& x) {
  return sum_f()(mean_normalize(x)) / (x.size() - 1);
}
...
SEXP roll_max(NumericVector x, NumericVector weights, SEXP pad) {
  return roll_vector_with(max_f(), x, weights, pad);
}

// [[Rcpp::export]]
SEXP roll_min(NumericVector x, NumericVector weights, SEXP pad) {
  return roll_vector_with(min_f(), x, weights, pad);
}

// [[Rcpp::export]]
SEXP roll_prod(NumericVector x, NumericVector weights, SEXP pad) {
  return roll_vector_with(prod_f(), x, weights, pad);
}
