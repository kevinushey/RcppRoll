roll_Rcpp_file <- function( fun, includes, depends, types ) {
  
  outFile <- file.path( getwd(), "src", "source_files.cpp" )
  conn <- file( outFile, open="a" )
  
  w <- function(...) {
    cat( paste0(..., "\n"), file=conn)
  }
  
  w1 <- function(...) {
    cat( paste0("\t", ..., "\n"), file=conn)
  }
  
  w2 <- function(...) {
    cat( paste0("\t\t", ..., "\n"), file=conn)
  }
  
  w3 <- function(...) {
    cat( paste0("\t\t\t", ..., "\n"), file=conn)
  }
  
  w4 <- function(...) {
    cat( paste0("\t\t\t\t", ..., "\n"), file=conn)
  }
  
  if( "NumericVector" %in% types ) {
    
    ## write out the numeric vector handler
    w("// [[Rcpp::export]]")
    w( "NumericVector roll_", fun, "_numeric_vector( NumericVector x, int n, NumericVector weights ) {" )
    w1()
    w1("int len = x.size();")
    w1("int len_out = len - n + 1;")
    w1()
    w1("NumericVector out = no_init( len_out );")
    w1()
    w1("for( int i=0; i < len_out; i++ ) {")  
    w2("out[i] = ", fun, "( x[ seq(i, i+n-1) ] * weights );")
    w1("}")
    w1()
    w1("return out;")
    w1()
    w("}")
    w()
    
  }
  
  if( "NumericMatrix" %in% types ) {
    
    ## write out the matrix handler
    w("// [[Rcpp::export]]")
    w("NumericMatrix roll_", fun, "_numeric_matrix( NumericMatrix A, int n, bool by_column, NumericVector weights ) {")
    w1()
    w1("int nRow = A.nrow();")
    w1("int nCol = A.ncol();")
    
    ## column apply
    w1("if( by_column ) {")
    w2()
    w2("NumericMatrix B( nRow - n + 1, nCol );")
    w2()
    w2("for( int j=0; j < nCol; j++ ) {")
    w3()
    w3("NumericVector tmp = A(_, j);")
    w3("for( int i=0; i < nRow - n + 1; i++ ) {")
    w4("B(i, j) = ", fun, "( tmp[ seq(i, i+n-1) ] * weights );")
    w3("}")
    w2("}")
    w1()
    w1("return B;")
    w1()
    
    ## row apply
    w1("} else {")
    w1()
    w2("NumericMatrix B( nRow, nCol - n + 1 );")
    w2()
    w2("for( int i=0; i < nRow; i++ ) {")
    w3()
    w3("NumericVector tmp = A(i, _);")
    w3("for( int j=0; j < nCol - n + 1; j++ ) {")
    w4("B(i, j) = ", fun, "( tmp[ seq(j, j+n-1) ] * weights );")
    w3("}")
    w2("}")
    w1()
    w1("return B;")
    w1()
    w1("}")
    w()
    w("}")
    
  }
  
  close( conn )
  
}