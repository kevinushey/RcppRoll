roll_Rcpp_file <- function( fun, includes, depends, types ) {
  ...
  if( "NumericMatrix" %in% types ) {
    ...
    w("// [[Rcpp::export]]")
    w("NumericMatrix roll_", fun, "_numeric_matrix( NumericMatrix A, int n, bool by_column ) {")
    w1()
    w1("int nRow = A.nrow();")
    w1("int nCol = A.ncol();")
    ...
    w4("for( int j=0; j < nCol - n + 1; j++ ) {")
    w3("B(i, j) = ", fun, "( tmp[ seq(j, j+n-1) ] );")
    w3("}")
    w2("}")
    w1()
    w1("return B;")
